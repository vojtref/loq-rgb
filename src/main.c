...
int main(int argc, char **argv)
{
	...
	log_debug("Mode: 0x%02x", OPTIONS.mode);
	log_debug("Speed: 0x%02x", OPTIONS.speed);
	log_debug("Brightness: 0x%02x", OPTIONS.brightness);
	log_debug("Zone colors:");
	for (size_t z = 0; z < 4; ++z) {
		log_debug("  %d: #%02x%02x%02x",
		          z,
